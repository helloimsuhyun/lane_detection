...
def find_line_fit(left_box_clean,right_box_clean, line_width= 300):
    x_current = None
    current_line = "MID"
    x_location = None
...
if __name__ == "__main__" : 
    ...
    while True:
        ...
        x_location, x_current , current_line = find_line_fit(L_pts,R_pts, line_width= 150)
        ...
        if prev_time is None:
            ...
            dt = max(1e-3, min(0.2, now - prev_time))  # 0.001~0.2s 범위로 제한ㅂ
        prev_time = now
                
        # --- 칼만 필터로 각도 추정 ---
        xxx = angle_kf.step(meas=x_location, dt=dt)
        angle = find_angle(bev, xxx , angle_deg)  # 조향각 관측값
