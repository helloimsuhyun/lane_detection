...
def find_base(img, past_L_base_idx, past_R_base_idx, basefindgrid_N = 20, thresh_score = 40000, find_base_width_idx = 3, mode = 0, show = False):
    ...
    ** 각 호출간의 basefindgrid_N이 동일해야함
    ...
    if (mode == 0 and past_L_base_idx != None) :
        ...
        if (basefind_idx_L_2 >= basefindgrid_midpoint) :  basefind_idx_L_2 = basefindgrid_midpoint
    ...
    if (mode == 0 and past_R_base_idx != None) :
        ...
        if (basefind_idx_R_1 <= basefindgrid_midpoint) :  basefind_idx_R_1 = basefindgrid_midpoint
    ...
    if (mode == 1 or past_L_base_idx == None or findbase_score[left_max_idx] <= thresh_score ) :
        left_max_idx = np.argmax(findbase_score[:basefindgrid_midpoint]) # 0 ~ basefindgrid_midpoint-1 까지 검사
        left_max_idx = int(left_max_idx)

    if (mode == 1 or past_R_base_idx == None or findbase_score[right_max_idx] <= thresh_score) :
        right_max_idx = np.argmax(findbase_score[basefindgrid_midpoint:]) # basefindgrid_midpoint ~ 끝까지 검사
...
def find_line_point(img,leftx_base,rightx_base, nwindows = 9, h_margin = 60, l_margin = 80, minpix = 40):
    """
    nwindows
    h_margin
    l_margin
...
def find_angle(img, L_coff, R_coff, lane_width = 450):
    H, W = img.shape
    ...
    lookahead_ys = [int(H * 0.3), int(H * 0.4), int(H * 0.6), int(H * 0.7)]
...
######################################전역변수
past_L_base_idx = None
past_R_base_idx = None
...
if __name__ == "__main__" : 
    ...
    while True:
        ...
        bev  = pre_process_2(mask , roi)
        show4 = debug_draw_roi(mask,roi)

        left_base, right_base , left_base_idx , right_base_idx = find_base(bev, 
                                        past_L_base_idx = past_L_base_idx,
                                        past_R_base_idx = past_R_base_idx,
                                        basefindgrid_N = 20, 
                                        thresh_score = 40000, 
        ...
        past_L_base_idx = left_base_idx
        past_R_base_idx = right_base_idx
        # Figure 및 subplot 설정
